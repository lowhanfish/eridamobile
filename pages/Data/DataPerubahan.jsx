...
import { View, Text, Image, TouchableOpacity, StyleSheet, ScrollView, Alert, TextInput, RefreshControl } from "react-native";
...
const DataPerubahan = () => {
    ...
    const kategoriOptions = [
        { id: '1', nama: 'Inovasi Layanan' },
        { id: '2', nama: 'Inovasi Proses' },
        { id: '3', nama: 'Inovasi Produk' },
        { id: '4', nama: 'Inovasi Tata Kelola' },
    ];
    ...
    const downloadFile = (data) => {
        ...
        const namaDokumen = data.judul || 'Dokumen Aksi Perubahan';
    ...
    const getData = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                urlx.URL_Perubahan + "/view",
                {
                    data_ke: page_first,
                    cari_value: cari_value,
                    tahun: tahun_filter || null,
                    ktgr: kategori_filter || null,
                },
                {
                    headers: {
            ...
            const sampleChartData = [
                { tahun: String(currentYear), count: Math.floor(Math.random() * 15) + 3 },
                { tahun: String(currentYear - 1), count: Math.floor(Math.random() * 15) + 8 },
                { tahun: String(currentYear - 2), count: Math.floor(Math.random() * 15) + 12 },
                { tahun: String(currentYear - 3), count: Math.floor(Math.random() * 15) + 6 },
                { tahun: String(currentYear - 4), count: Math.floor(Math.random() * 15) + 10 },
            ];
            setChartData(sampleChartData);
        } catch (err) {
            console.log('Get data perubahan error:', err);
            // Set sample chart data on error
            const sampleChartData = [
                { tahun: String(currentYear), count: 8 },
                { tahun: String(currentYear - 1), count: 15 },
                { tahun: String(currentYear - 2), count: 12 },
                { tahun: String(currentYear - 3), count: 6 },
                { tahun: String(currentYear - 4), count: 10 },
            ];
            setChartData(sampleChartData);
        } finally {
    ...
    const getKategoriNama = (kategori_id) => {
        const kat = kategoriOptions.find(k => k.id === kategori_id);
        return kat ? kat.nama : '-';
    };
    ...
    const getChartData = async () => {
        ...
        try {
            ...
            console.log("Chart perubahan error:", err);
            setChartData([]);
        } finally {
                ...
                <View style={{ flex: 1, paddingBottom: 72 }}>
                    <View style={stylex.pageTitleContainer}>
                        <View style={[stylex.pageTitleItemContainer, { justifyContent: 'center' }]}>
                            <View >
                                <Text style={stylex.textTitleList}>DATA AKSI PERUBAHAN</Text>
                    ...
                    <View style={[stylex.InputContainer, styles.searchContainer]}>
                        <Text style={stylex.inputText1}>Cari Data Aksi Perubahan</Text>
                    ...
                    <View style={styles.chartContainer}>
                        <Text style={styles.chartTitle}>Jumlah Data Aksi Perubahan per Tahun</Text>
                                    ...
                                    <View style={[styles.docCard, stylex.shaddow]}>
                                        <View style={styles.docIconContainer}>
                                            <Image 
                                                ...
                                                source={require('../assets/images/aksi.png')}
                                            />
                                        </View>
                                            ...
                                            <View style={styles.docMeta}>
                                                ...
                                                <View style={[styles.docMetaItem, { marginLeft: 12 }]}>
                                                    <Text style={[styles.docMetaText, { color: '#EFD06D' }]}>
                                                        {getKategoriNama(data.kategori_id)}
                                                    </Text>
                                                </View>
                                            </View>
                                        </View>
